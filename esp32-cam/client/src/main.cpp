...
bool connectToServer() {
  ...
  /*dùng để đo tổng thời gian kết nối + truyền dữ liệu + ngắt kết nối*/
...
void loop() {
  ...
  /*nhận giá trị từ Serial rồi truyền dữ liệu tương ứng qua BLE*/
  ...
  if (connected) {
    ...
    if (nodeVote != 0){
      Serial.printf("Input: %d\n", nodeVote); /*in ra giá trị ASCII được nhập*/
      switch (nodeVote) {
        case 65: case 97:
          pRemoteChar_1->writeValue("1");
      ...
      delay(100);
      std::string receivedValue = pRemoteChar_1->readValue();
