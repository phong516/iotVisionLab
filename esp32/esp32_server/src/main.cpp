...
#define SERVICE_UUID          "0xA0"
#define CHARACTERISTIC_UUID_1 "0xA1"
#define CHARACTERISTIC_UUID_2 "0xA2"
...
void loop() {
    ...
    if (deviceConnected) {
     // use pCharacteristic_1 to get the value from client, then Serial.print it out
      ...
      if (value.length() > 0) {
        ...
        Serial.println();
              ...
              // use pCharacteristic_2 to send the string "Received: " + value to client
        pCharacteristic_2->setValue(value);
        pCharacteristic_2->notify();
        delay(100);
      }
      digitalWrite(9, LOW);
      pCharacteristic_1->setValue("");
